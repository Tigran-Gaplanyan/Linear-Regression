...
class CustomLinearRegression:
    ...
    def init_weights(self, input_size: int, output_size: int):
        ...
        self.W = 0.01 * np.random.randn(input_size, output_size)  # TODO
        self.b = np.zeros((1, output_size))  # TODO
    ...
    def fit(self, X: np.ndarray, y: np.ndarray, num_epochs: int = 1000, lr: float = 0.001):
        ...
        for _ in range(num_epochs):
            ...
            b_grad = 2/m * np.sum(preds - y)  # TODO
            W_grad = 2/m * X.T @ (preds - y) + 2/m * self.C * self.W  # TODO
            self.W = self.W - lr * W_grad  # TODO
